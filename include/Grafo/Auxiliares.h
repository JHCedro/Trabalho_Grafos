...
uint* leituraIntanciaSteiner(string nomeArq, Grafo *&G, bool GHash = false){
    ...
    if(GHash) G = new GrafoHash((uint) (n_nos * 1.5), false);
...
void desempenhoHeuristicas(int args, char **argv){
    string arquivo = string(argv[1]);
    usint heuristica = (usint) atoi(argv[2]);
    int semente = atoi(argv[3]);
    uint nTSementes = (uint) atoi(argv[4]);
    ...
    tempo = (unsigned long long int) clock();
    if(heuristica==1)
