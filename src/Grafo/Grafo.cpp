...
bool Grafo::mesmaComponenteFortementeConexa(uint id1, uint id2){
    ...
    return i1 != NULL && i2 != NULL && mesmaComponenteFortementeConexa(i1, i2);
}
...
bool Grafo::ehNoArticulacao(uint id){
    No* no=buscaNo(id);
    return no != NULL && this->ehNoArticulacao(buscaNo(id));
}
...
Grafo* Grafo::clone(){
    ...
    for(itInicio(); !itEhFim(); itProx()){
        G->insereNo(this->getIt()->getID())->setIdArvore((unsigned int) idArvore);///id auxiliar para algoritmo de kruskal
        idArvore++;
    }
...
double Grafo::gulosoRandomizadoReativoSteiner(uint idTerminais[], uint tam, int semente, bool imprimeSolucao){
    ...
    for (int i = 0; i < max_iteracoes; i++){
        ...
        if( i < bloco_iteracoes )
            /// no primeiro bloco de iteracoes todos os alphas s�o testados igualmente
            alpha = i % m;
        else
            alpha = (uint) distribuicao(generator);
